...
def get_heurestic(source_cord, destination_cord):
    return calculate_distance(source_cord, destination_cord)


def calculate_distance(source_cord, destination_cord):
    x_square_diff = (source_cord[0] - destination_cord[0]) ** 2
    y_square_diff = (source_cord[1] - destination_cord[1]) ** 2
...
def shortest_path(M, start, goal):
    ...
    if graph is None or graph.roads[start] is None or len(graph.roads[start]) < 1:
        return None
    if start == goal:
        return [start]
    ...
    goal_cord = graph.intersections[goal]

    g_cost = 0
    h_cost = get_heurestic((graph.intersections[start]), goal_cord)
    f_cost = h_cost + g_cost

    queue.insert((start, g_cost, h_cost), f_cost)

    while not queue.isEmpty():

        current_node, g_cost, h_cost = queue.remove()
        current_node_cord = graph.intersections[current_node]

        if current_node == goal:
            break

        for neighbor in graph.roads[current_node]:

            neighbor_cord = graph.intersections[neighbor]

            neighbor_g_cost = g_cost + calculate_distance(current_node_cord, neighbor_cord)
            neighbor_h_cost = get_heurestic(neighbor_cord, goal_cord)
            ...
            if neighbor in shortest_dist:
                if shortest_dist[neighbor] > neighbor_f_cost:
                    ...
                    queue.insert((neighbor, neighbor_g_cost, neighbor_h_cost), neighbor_f_cost)
    ...
    while current_node != start:
        parent = parent_dict[current_node]
        path.append(parent)
        current_node = parent
    path.reverse()
    return path
